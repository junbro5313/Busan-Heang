...
int len, prob ; // 열차 길이, 확률
int cy, zy, my; // 시민, 좀비, 마동석 위치
int dm; // dongseok move
int m_act; // dongseok action
int zturn = 1; // 좀비 이동 불가 턴 결정
int c_aggro = 1; // 시민 어그로
int m_arr[2] = { 1,0 }; // {마동석 어그로,스테미나}

void intro();
void init(int,int,int,int); // 열차 상태 출력 함수
void display_status(int, int, int, int, int, int); // 시민,좀비 상태 출력 함수
void m_status(int, int, int, int); // 마동석 상태 출력 함수
int what_they_do(int,int,int,int); // 시민, 좀비 행동 상태 출력 함수
int what_ru_do(int[],int,int); // 마동석 행동 상태 출력 함수
int outtro_success();
int outtro_fail();
